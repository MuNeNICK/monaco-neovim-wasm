...
interface MsgpackValueMap { [key: string]: MsgpackValue; }
...
function write(val: MsgpackValue, out: number[]): void {
  ...
  if (typeof val === "object") {
    writeMap(val as Record<string, MsgpackValue>, out);
    return;
  }
  throw new Error("Unsupported type in msgpack encode");
}

function writeNumber(num: number, out: number[]): void {
...
function writeBigInt(bi: bigint, out: number[]): void {
  const buf = new ArrayBuffer(8);
  const view = new DataView(buf);
...
function writeString(str: string, out: number[]): void {
...
function writeBinary(bytes: Uint8Array, out: number[]): void {
  const len = bytes.length;
  if (len <= 0xff) {
    out.push(0xc4, len);
  } else if (len <= 0xffff) {
...
function writeMap(map: Record<string, MsgpackValue>, out: number[]): void {
...
function writeExt(ext: MsgpackExt, out: number[]): void {
...
function isExt(val: MsgpackValue): val is MsgpackExt {
  return Boolean(
    val
    && typeof val === "object"
    && typeof (val as MsgpackExt).type === "number"
    && (val as MsgpackExt).data instanceof Uint8Array,
  );
}

export class Decoder {
  private buffer: Uint8Array;
  private readonly onMessage: (value: MsgpackValue) => void;
  ...
  push(chunk: Uint8Array): void {
    ...
    while (offset < this.buffer.length) {
      try {
        ...
        const incomplete = (err as Error & { incomplete?: boolean }).incomplete;
        if (incomplete) break;
        throw err;
      }
    }
...
function needMore(): Error & { incomplete: true } {
  const err = new Error("incomplete msgpack buffer") as Error & { incomplete: true };
  err.incomplete = true;
  return err;
}

function decodeValue(buf: Uint8Array, offset: number): { value: MsgpackValue; nextOffset: number } {
...
function decodeBin(buf: Uint8Array, offset: number, len: number, header: number) {
...
function decodeString(buf: Uint8Array, offset: number, len: number, header: number) {
  const start = offset + header;
  const end = start + len;
  if (end > buf.length) throw needMore();
...
function decodeArray(buf: Uint8Array, offset: number, count: number, header: number) {
...
function decodeMap(buf: Uint8Array, offset: number, count: number, header: number) {
  let off = offset + header;
  const obj: Record<string, MsgpackValue> = {};
  for (let i = 0; i < count; i += 1) {
...
function decodeFixExt(buf: Uint8Array, offset: number, len: number) {
  const start = offset + 2;
  const end = start + len;
  if (end > buf.length) throw needMore();
  const data = buf.slice(start, end);
  const value: MsgpackExt = { type: buf[offset + 1], data };
  return { value, nextOffset: end };
}

function decodeExt(buf: Uint8Array, offset: number, len: number, header: number) {
  const typeId = buf[offset + header - 1];
  const start = offset + header;
  const end = start + len;
  if (end > buf.length) throw needMore();
  const data = buf.slice(start, end);
  const value: MsgpackExt = { type: typeId, data };
  return { value, nextOffset: end };
}

function decodeUInt(buf: Uint8Array, offset: number, size: number, bigint: boolean) {
...
function decodeInt(buf: Uint8Array, offset: number, size: number) {
  const start = offset + 1;
  const end = start + size;
  if (end > buf.length) throw needMore();
  const view = new DataView(buf.slice(start, end).buffer);
...
function decodeFloat(buf: Uint8Array, offset: number, size: number) {
  const start = offset + 1;
  const end = start + size;
  if (end > buf.length) throw needMore();
  const view = new DataView(buf.slice(start, end).buffer);
...
function readLen(buf: Uint8Array, offset: number, size: number) {
  const start = offset;
  const end = start + size;
  if (end > buf.length) throw needMore();
